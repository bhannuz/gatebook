...
function renderAnPayTable(filterType, selMonth, selYear, selBlock) {
  ...
  tbody.innerHTML = rows.map((f,i) => {
    ...
    return `<tr
      ...
      onmouseover="this.style.background='var(--indigo-bg)'"
      onmouseout="this.style.background=''">
      <td style="width:20%;max-width:0;padding:8px 10px;font-weight:800;color:var(--indigo);font-size:12px;border-bottom:1px solid var(--border);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;vertical-align:middle">${f.flatId}</td>
      <td style="width:36%;max-width:0;padding:8px 10px;border-bottom:1px solid var(--border);overflow:hidden;vertical-align:middle">
      ...
      <td style="width:18%;padding:8px 10px;text-align:center;border-bottom:1px solid var(--border);vertical-align:middle">${statusIcon[s]||statusIcon.pending}</td>
      <td style="width:26%;max-width:0;padding:8px 10px;text-align:right;font-weight:800;color:${balColor};font-size:12px;border-bottom:1px solid var(--border);white-space:nowrap;overflow:hidden;text-overflow:ellipsis;vertical-align:middle">${f.due?inr(Math.abs(bal)):'—'}</td>
    </tr>`;
  }).join('');
  ...
  tbody.innerHTML += `<tr style="background:var(--surface3);border-top:2px solid var(--border2)">
    <td style="width:56%;padding:8px 10px;font-size:11px;font-weight:800;color:var(--text2)" colspan="2">Total — ${rows.length} flats</td>
    <td style="width:18%;padding:8px 10px;text-align:center;font-size:10px;color:var(--muted)">—</td>
    <td style="width:26%;padding:8px 10px;text-align:right;font-weight:800;color:${totBal>0?'var(--red)':'var(--green)'};font-size:13px;white-space:nowrap">${inr(Math.abs(totBal))}</td>
  </tr>`;
}
